...
Добавьте Streamable HTTP URL как MCP-коннектор и завершите OAuth. После подключения попросите агента прочитать нужный навык через MCP и подтвердить актуальный пакет вызовом `ack_skills` при необходимости.
